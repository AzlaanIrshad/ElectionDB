  <template>
    <div class="flex">
      ...
      <div class="w-1/4 p-4 border-r border-gray-300">
        ...
        <div class="mb-4">
          <label for="year" class="block font-bold mb-2">Selecteer Jaar:</label>
          <select
              id="year"
              v-model="selectedYear"
              @change="onYearChange"
              class="bg-white dark:bg-gray-900 border border-gray-300 rounded px-2 py-1 w-full"
          >
            <option
  ...
  export default {
    ...
    methods: {
      ...
      /**
       * Genereert kleuren voor elke partij uit een vooraf gedefinieerde set
       */
      generatePartyColors() {
        const uniqueParties = [
          ...new Set(this.electionData.map((item) => item.leadingParty)),
        ];
        let colorIndex = 0;
        uniqueParties.forEach((party) => {
          if (!this.partyColors[party]) {
            this.partyColors[party] =
                this.predefinedColors[colorIndex % this.predefinedColors.length];
            colorIndex++;
          }
        });
      },
        ...
        this.electionData.forEach((transaction) => {
          ...
          if (
              ...
              (!this.selectedParties.length ||
                  this.selectedParties.includes(leadingParty))
          ) {
      ...
      /**
       * Wijzigt het geselecteerde jaar en haalt nieuwe resultaten op
       */
      onYearChange() {
        this.fetchElectionResults();
      },
