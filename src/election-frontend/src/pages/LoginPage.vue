  ...
  <div class="login-container flex items-center justify-center min-h-screen dark:bg-gray-900">
    <div class="login-box bg-white dark:bg-gray-800 shadow-lg rounded-lg p-8 max-w-sm w-full">
      ...
      <form @submit.prevent="loginUser">
        ...
        <p class="error-message text-red-500 text-center mt-4 text-sm" v-if="errorMessage">{{ errorMessage }}</p>
      </form>
...
export default {
  ...
  methods: {
    ...
    validateForm() {
      const errors = {};
      if (!this.email) errors.email = 'E-mailadres is verplicht.';
      else if (!/\S+@\S+\.\S+/.test(this.email))
        errors.email = 'Voer een geldig e-mailadres in.';
      if (!this.password) errors.password = 'Wachtwoord is verplicht.';
      this.errors = errors;
